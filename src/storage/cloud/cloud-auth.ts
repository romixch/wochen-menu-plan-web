...
const requestAccessToken = async (oneTimeToken: string): Promise<boolean> => {
  const email = await getAccountEmail();
  try {
    const accessTokenResponse = await fetch(`${BASE_URL}register/${email}/${oneTimeToken}`, { method: 'GET' });
...
const getLoginState = async (): Promise<LoginState> => {
  ...
  const loginStateValueMigrated = loginStateValue === null ? await migrateLoginState() : loginStateValue
...
export const getShareData = async (): Promise<string> => {
  return JSON.stringify({ email: await getAccountEmail(), accessToken: await getAccessToken() })
}
...
const onLoginListeners: ((previousLoginEmail: string | null, nextLoginEmail: string | null) => Promise<void>)[] = [];
const addOnLoginListener = (handler: (previousLoginEmail: string | null, nextLoginEmail: string | null) => Promise<void>) => {
  onLoginListeners.push(handler);
}
const fireOnLoginListener = async (previousLoginEmail: string | null, nextLoginEmail: string | null) => {
  const promises = onLoginListeners.map(async listener => await listener(previousLoginEmail, nextLoginEmail))
